export function setupEventUI(getWorld,{night=false}={}){
 ...
 function apply(){
  document.body.classList.toggle('night',isNight);light.setAttribute('aria-pressed',String(isNight));light.setAttribute('aria-label',isNight?'Switch to daylight':'Switch to moonlight');
  ...
  motion.setAttribute('aria-pressed',String(paused));motion.setAttribute('aria-label',paused?'Resume village animation':'Pause village animation');motion.querySelector('span').textContent=paused?'▶':'Ⅱ';
