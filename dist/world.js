...
export async function createWorld(canvas,pins,onReady,onError,onNavigate=()=>{},initial={}){
 ...
 function move(event){if(!pointer.down){const hit=hitTest(event);pointer.hover=hit||'';canvas.classList.toggle('over-landmark',!!hit);if(progress<.35){dirty=true;wake();}return;}const dx=event.clientX-pointer.x,dy=event.clientY-pointer.y;if(event.pointerType==='touch'&&!pointer.dragged&&Math.abs(event.clientY-pointer.startY)>Math.abs(event.clientX-pointer.startX))return;if(Math.abs(event.clientX-pointer.startX)+Math.abs(event.clientY-pointer.startY)>6){pointer.dragged=true;if(!canvas.hasPointerCapture(event.pointerId))canvas.setPointerCapture(event.pointerId);}if(pointer.dragged){const factor=cameraPose(progress,width,height,zoom).height/height;panXGoal=clamp(panXGoal-dx*factor*.707+dy*factor*.866,-15,15);panZGoal=clamp(panZGoal+dx*factor*.707+dy*factor*.866,-15,15);dirty=true;wake();}pointer.x=event.clientX;pointer.y=event.clientY;}
 function release(event){const click=pointer.down&&!pointer.dragged&&event.type!=='pointercancel';pointer.down=false;canvas.classList.remove('dragging');if(canvas.hasPointerCapture(event.pointerId))canvas.releasePointerCapture(event.pointerId);if(click){const hit=hitTest(event);if(hit)onNavigate(hit);}}
 ...
 canvas.addEventListener('pointerdown',down);canvas.addEventListener('pointermove',move);canvas.addEventListener('pointerup',release);canvas.addEventListener('pointercancel',release);canvas.addEventListener('keydown',key);
